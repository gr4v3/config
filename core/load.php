...
class Include_helper {
    ...
    function file($filename = NULL) {
        ...
        if (array_key_exists($filename, $this->caller->includes)) return TRUE;
        $component_path = INCLUDES;
        if (strpos($filename, DS)) $realfilename = end(explode(DS, $filename));
        else $realfilename = $filename;
        $file = $component_path . DS . $filename;
        if (is_dir($component_path)) {
            if (is_file($file . '.php')) $file_input = $file . '.php';
            else if (is_file($file)) $file_input = $file;
    ...
    function folder($foldername = NULL) {
        ...
        if (array_key_exists($foldername, $this->caller->includes)) return TRUE;
        $component_path = INCLUDES;
...
class Load_helper {
    ...
    protected function _cache($caller = NULL, $realfilename = NULL) {
        ...
        if (property_exists($caller, $realfilename)) {
            if (property_exists($caller, 'log')) $caller->log->write('cache', "class $realfilename is cached from father class " . get_class($caller));
            return $caller->{$realfilename};
        } else if (property_exists($caller, 'father')) return $this->_cache($caller->father, $realfilename);
    }
    protected function _run($type = NULL, $params = NULL, $return = FALSE) {
        if (empty($type) || empty($params)) return FALSE;
        if (strpos($params, DS)) $realfilename = end(explode(DS, $params));
        else $realfilename = $params;
        // check if the class allready exist in cache
        $class_object = $this->_cache($this->caller, $realfilename);
        if ($class_object) {
            if ($return) return $class_object;
            else {
        ...
        if ( ! class_exists($params_class_name)) $this->_check($type , $params);
