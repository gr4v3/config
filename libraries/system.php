...
class System_Helper {
    ...
    public function search($handle = NULL, $index = NULL) {
        ...
        if ($line) {
            ...
            if ( ! empty($row[$index])) return $row[$index];
            else return $this->search($handle, $index);
        } else return FALSE;
    }
    public function file($params = NULL, $rename = NULL) {
        ...
        switch ($this->mode) {
            ...
            case 'read':    
                if (is_file($params)) return file_get_contents($params);
                else return NULL;
            case 'search':    
                ...
                return $line;    
            case 'write':
                if (empty($rename)) $rename = array();
                if (is_file($params)) return file_put_contents($params, json_encode($rename));
                else return FALSE;
            case 'update':
    ...
    public function folder($params = NULL, $rename = NULL) {
        ...
        switch ($this->mode) {
            ...
            case 'write':
            case 'create':
                if (!is_dir($params)) return mkdir($params, 0755);
                else return TRUE;
            case 'search':    
            case 'read':    
                $dir = @scandir($params);
                ...
                foreach($dir as $value) {
                    if (!in_array($value,array('.','..','.svn'))) $results[] = $value;
                }
                return $results;
        }
    }
...
class System {
    ...
    function normalize_array($arr = NULL, $index = NULL) {
        if (empty($arr) || empty($index) || !is_array($arr)) return $arr;
        $result = array();
