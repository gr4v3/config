...
class Url extends Load {
    ...
    private function enviroment() {
        ...
        if (strpos($REQUEST_URI, $this->project())) return 'alias';
        else return 'vhost';
    }
    public function base() {
        $enviroment = $this->enviroment();
        $this->log->write('enviroment', $enviroment);
        if ($enviroment == 'alias') return 'http://'.$_SERVER['HTTP_HOST'].'/'.$this->project().'/';
        else return 'http://'.$_SERVER['HTTP_HOST']. '/';
    }
    public function request() {
        $enviroment = $this->enviroment();
        if ($enviroment == 'alias') return str_replace('/' . $this->project(),'',$_SERVER['REQUEST_URI']);
