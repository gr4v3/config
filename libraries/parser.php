...
class Parser_Helper {
    ...
    public function css($params = NULL, $declaration = FALSE) {
        ...
        if ($declaration) $this->caller->css[] = '<style type="text/css">'.$params.'</style>';
        else $this->caller->css[] = $this->link($this->caller->css_base_url . '/' . $params, $declaration);
    }
    public function js($params = NULL, $declaration = FALSE) {
        if (empty($params)) return FALSE;
        if ($declaration) $this->caller->js[] = '<script type="text/javascript">'.$params.'</script>';
        else $this->caller->js[] = $this->script($this->caller->js_base_url . '/' . $params, $declaration);
    }
...
class Parser extends Load {
    ...
    function template($params = NULL, $return = FALSE) {
        ...
        if ($return) {
            ...
            $file = NULL;
            if (is_file($this->views_path . $params . '.php')) $file = $params . '.php';
            else if (is_file($this->views_path . $params )) $file = $params;
