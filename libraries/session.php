...
class Session extends Load {
    ...
    public function delete($session_id = NULL) {
        if (empty($session_id)) return $this->index();
        $this->godsql->session->where(array('session_id' => $session_id));
    ...
    private function create($session_id = NULL, $extra = NULL) {
        ...
        if ( ! empty($extra)) $session_data = array_merge($session_data, $extra);
    ...
    private function redirect($session_id = NULL, $redirect = NULL) {
        ...
        } else header('Location: '.$this->url->base().'login');
    }
    public function check() { 
        ...
        if ($uri && empty($action)) return FALSE;
        ...
        if (! $session_user) {
            ...
            if (empty($results))  $firsttime = '/firsttime';
            if ($this->url->request() == '/login') $this->create($session_id, array('redirect' => $this->config->default_main_controller_method));
            else $this->create($session_id, array('redirect' => ltrim($this->url->request(),'/')));
    ...
    public function present() {
        ...
        $session_id = session_id();
        $session_user = $this->read($session_id);
        if ($session_user && $session_user['logged'] == 2) header('Location: '.$session_user['redirect']);
    }
}
?>