...
class Namespaces extends load {
    ...
    public function dir() {
        ...
        if (empty($application) || empty($environment)) return FALSE;
    ...
    public function create() {
        ...
        if ( ! empty($cancel)) header('Location: /' . $this->config->default_main_controller_method);
        if (! empty($create)) {
            $application = $this->request->post('application');
            $environment = $this->request->post('environment');
            ...
            if ($handle) {$result_str = 'success!';fclose($handle);} else $result_str = 'failed!';
    ...
    public function update() {
        ...
        if ( ! empty($namespaces)) {
            ...
            if ( ! empty($remove)) {
                if ( ! empty($remove)) unset($file_content_assoc[$remove]);
            }
            
            $this->system->write->file($path, $file_content_assoc);
                    
            if (empty($file_content_assoc)) $file_content_assoc = array();
